...
def analyze_particles_and_save(image_path, roi_zip_path, output_dir):
    ...
    messagebox.showinfo(title="Scale", message=f"[INFO] 스케일 바: {scale_bar_px}px = {real_length}µm → {scale_ratio:.3f} µm/px")
    ...
    try:
        ...
        messagebox.showinfo(f"탐색기 열기 실패: {e}")
        
    messagebox.showinfo(title="완료", message=f"완료: {len(results)}개 ROI 분석 및 저장 완료 → {output_dir}")
...
if __name__ == "__main__":
    ...
    if image_path and roi_zip_path and output_dir:
        ...
        messagebox.showinfo(title="에러", message="입력 또는 출력 경로가 선택되지 않았습니다.")